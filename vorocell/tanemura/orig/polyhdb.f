...
C
      ...
      INTEGER NXN
      REAL*8 TBEG, TEND, TT00, TT11
      REAL*8 DT1, DT2, DT3, DT4, DT5
      REAL*8 DT41, DT42, DT43, DT44, DT45, DT46, DT47, DT48
      LOGICAL mFLAG
      INTEGER myCNT, myNMX
      REAL*8 myFLOP
      DT1 = 0
...
260   isjk(1) = 1;
      nn = 0;
      call plane (ia, ib, c1,c2,c3)
      fNum = -1.0e10
      t1 = 0
      t2 = 0
      t3 = 0
      tt0 = c1*x(ic) + c2*y(ic) + c3*z(ic)
      myCNT = myCNT + 1
      myNMX = myNMX + nmx
...
c slow version
      do j=2,nmx
        ...
        if (mFLAG) then
          ...
          yd = y(j)
          zd = z(j)
          tt = c1*xd + c2*yd + c3*zd
c         if (abs(tt).le.eps) stop 'errorYY'
          myFLOP = myFLOP + 7
          if (tt*tt0 < 0.0) then
            tt = xd*(xd+t1) + yd*(yd+t2) + zd*(zd+t3) + fNum
            myFLOP = myFLOP + 13
c            if (abs(tt).lt.eps) stop 'errorYYY'
            if (tt < 0.0) then
...
c fast version
      do j=2,nmx
        ...
        zd  = z(j)
        tt  = c1*xd + c2*yd + c3*zd
        tt1 = xd*(xd+t1) + yd*(yd+t2) + zd*(zd+t3) + fNum
        myFLOP = myFLOP + 20
        mFLAG=(j.ne.ia).and.(j.ne.ib).and.(j.ne.ic).and.(lc(j).eq.0)
...
C
 ...
 9999 CONTINUE
      TEND = rtc()
      print *, "spent:", TEND-TBEG, " seconds NXN=", NXN
      print *, "DT1= ", DT1
      print *, "DT2= ", DT2
      print *, "DT3= ", DT3
      print *, "DT4= ", DT4
      ...
      print *, "  DT47= ", DT47
      print *, "  DT48= ", DT48
      print *, "   SUM= ", DT40+DT41+DT42+DT43+DT44+DT45+DT46+DT47+DT48
      print *, "DT5= ", DT5
      print *, "TOTAL= ", DT1+DT2+DT3+DT4+DT5

      print *, "net rate: ", NP*NXN/(TEND-TBEG), "cell/sec"
      print *, "1:", NP*NXN/DT1
      print *, "2:", NP*NXN/DT2
      print *, "3:", NP*NXN/DT3
      print *, "4:", NP*NXN/DT4
      print *, "5:", NP*NXN/DT5
      print *, "myCNT:" ,myCNT*1.0/NXN
